...
const Waveform: React.FC<{ type: 'sine' | 'square' | 'rf'; active: boolean; color: string }> = ({ type, active, color }) => {
  ...
  return (
    <div className={`w-full h-full overflow-hidden ${active ? 'opacity-80' : 'opacity-100'}`}>
      <svg viewBox="0 0 120 30" preserveAspectRatio="none" className="w-full h-full">
        ...
        {type === 'square' && (
          <path
            ...
            stroke={color}
            strokeWidth={active ? "1" : "0.8"}
            className={active ? "animate-[wave_0.5s_linear_infinite]" : "animate-[wave_3s_linear_infinite]"}
          />
        )}
        {type === 'rf' && (
          <path
            ...
            stroke={color}
            strokeWidth={active ? "1" : "0.8"}
            className={active ? "animate-[wave_0.3s_linear_infinite]" : "animate-[wave_2s_linear_infinite]"}
          />
        )}
      </svg>
      <style>{`
...
const FrequencyBand: React.FC<{ plan: PricePlan; index: number; isSelected: boolean; onSelect: () => void }> = ({ plan, index, isSelected, onSelect }) => {
  ...
  const inactiveColor = INACTIVE_COLORS[index % INACTIVE_COLORS.length];

  return (
    <div
      ref={bandRef}
      onClick={onSelect}
      className={`relative cursor-pointer transition-all duration-700 flex flex-col items-center group shrink-0 ${isSelected ? 'scale-105 z-20' : 'opacity-70 hover:opacity-100'}`}
      style={{ width: `${width}px` }}
    >
      {/* Label Tooltip */}
      <div className={`absolute -top-36 left-1/2 -translate-x-1/2 w-48 sm:w-56 bg-slate-900 border border-yellow-500/50 p-3 sm:p-4 rounded-lg transition-all duration-500 z-50 ${isSelected ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
      ...
      <div
        className={`w-full relative transition-all duration-700 rounded-t-sm border-x border-t ${isSelected ? 'border-yellow-500 bg-yellow-500/10 shadow-[0_0_20px_rgba(251,191,36,0.1)]' : 'bg-slate-800/20'}`}
        style={{
          height: `${height}px`,
          borderColor: isSelected ? '#FBBF24' : `${inactiveColor}66` // 66 is 40% opacity in hex
        }}
      >
        <div className="absolute inset-0 py-2 sm:py-4 flex flex-col gap-2 sm:gap-4">
          <Waveform type={index % 3 === 0 ? 'sine' : index % 3 === 1 ? 'square' : 'rf'} active={isSelected} color={isSelected ? '#FBBF24' : inactiveColor} />
          <Waveform type={index % 3 === 0 ? 'rf' : index % 3 === 1 ? 'sine' : 'square'} active={isSelected} color={isSelected ? '#FBBF24' : inactiveColor} />
          <Waveform type={index % 3 === 0 ? 'square' : index % 3 === 1 ? 'rf' : 'sine'} active={isSelected} color={isSelected ? '#FBBF24' : inactiveColor} />
        </div>
      ...
      {/* Base Value */}
      <div
        className="mt-3 font-mono text-[8px] uppercase tracking-widest text-center transition-colors duration-500"
        style={{ color: isSelected ? '#FBBF24' : `${inactiveColor}aa` }}
      >
